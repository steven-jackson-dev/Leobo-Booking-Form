<?php
// Admin Handler: Load booking system for admin pages (must load before admin_menu hook)
add_action('init', function() {
    // Load for admin area
    if (is_admin() && !isset($GLOBALS['leobo_booking_system'])) {
        ...
        if (file_exists($booking_path . '/CustomBookingSystem.php')) {
            ...
            if (class_exists('LeoboCustomBookingSystem')) {
                ...
                error_log('=== BOOKING SYSTEM LOADED FOR ADMIN ===');
            }
        }
    }
...
// Frontend conditional loading - for non-admin pages
add_action('wp', function() {
    global $post;
    
    // Check if booking system should load (only for frontend, not admin)
    $should_load = false;
