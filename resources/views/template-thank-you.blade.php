...
<header class="leo-header bg-transparent absolute top-0 left-0 z-[5] w-full h-[105px] flex items-center justify-between px-7 md:px-10">
  ...
  <section class="leo-menu-modal fixed top-0 left-0 h-[100dvh] w-full z-[5] bg-maroon mobile-menu" style="display:none;">
    <img src="@asset('images/charcoal-bg-high-res.png')" loading="eager" alt="" class="leo-menu-modal-bg-img object-cover absolute top-0 left-0 w-full h-full z-[6]">
    ...
    <div class="leo-top-bar relative z-[8] flex justify-between items-center h-[105px] px-7 md:px-10">
      <img src="@asset('images/icons/close-white.svg')" loading="lazy" alt="" class="leo-close-menu-modal hover:cursor-pointer" />
              ...
              @foreach ($header_videos as $item)
                <div id="{!! $item['video_id'] !!}" class="leo-menu-modal-media-wrapper-inner relative h-full w-full @if($iteration > 1) hidden @endif">
                  ...
                  @if(!empty($item['video_poster_image']))
                    <img src="{!! $item['video_poster_image']['url'] !!}" loading="eager" alt="{!! $item['video_poster_image']['alt'] !!}" class="leo-menu-modal-video__poster absolute z-[1] top-0 left-0 w-full h-full object-cover object-center">
                  @endif
                </div>
                @php $iteration += 1 @endphp
              @endforeach
            ...
            @if(!empty($header_videos))
              <div id="{!! $header_videos[0]['video_id'] !!}" class="leo-menu-modal-media-wrapper-inner relative h-full w-full">
                ...
                @if(!empty($header_videos[0]['video_poster_image']))
                  <img src="{!! $header_videos[0]['video_poster_image']['url'] !!}" loading="eager" alt="{!! $header_videos[0]['video_poster_image']['alt'] !!}" class="leo-menu-modal-video__poster absolute z-[1] top-0 left-0 w-full h-full object-cover object-center">
                @endif
              </div>
            @endif
          </div>
      ...
      <section class="leo-hero h-[100dvh] relative w-full">
          <div class="leo-hero-content absolute top-0 left-0 h-full w-full z-[3] flex flex-col  px-7 md:px-10 py-20 md:py-10 items-center justify-center">
              <div class="bannerTxt_anim">
                  @if(!empty($thankyou_heading))
                      <h1 class="leo-hero-heading text-center opacity-0">{!! $thankyou_heading !!}</h1>
                  @endif
                  @if(!empty($thankyou_subheading))
                      <div class="leo-content text-lightSand/80 opacity-0 max-w-[400px] text-center font-cta font-normal mt-5">
                          <p>{!! $thankyou_subheading !!}</p>
                      </div>
                  @endif
                  @if(!empty($thankyou_cta))
                      <div class="leo-btn-wrapper mt-8 opacity-0 flex justify-center w-full">
                          <a href="{!! $thankyou_cta['url'] !!}" target="{!! $thankyou_cta['target'] !!}" class="leo-btn-primary">{!! $thankyou_cta['title'] !!}</a>
                      </div>
                  @endif
              </div>
          </div>
          <div class="leo-background absolute top-0 left-0 h-full w-full z-[1]">
              ...
              @if(!empty($thankyou_background_image))  
                  <img src="{!! $thankyou_background_image['url'] !!}" loading="eager" alt="{!! $thankyou_background_image['alt'] !!}" class="leo-background-img absolute z-[1] top-0 left-0 w-full h-full object-cover object-center hidden md:block">
              @endif
              @if(!empty($thankyou_mobile_background_image))  
                  <img src="{!! $thankyou_mobile_background_image['url'] !!}" loading="eager" alt="{!! $thankyou_mobile_background_image['alt'] !!}" class="leo-background-img--mobile absolute z-[1] top-0 left-0 w-full h-full object-cover object-center md:hidden">
              @endif
          </div>
      </section>
