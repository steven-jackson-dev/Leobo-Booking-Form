<header class="leo-header absolute top-0 left-0 z-[5] w-full h-[105px] flex items-center justify-between px-7 md:px-10">
  ...
  <a href="/" class="leo-main-logo-link absolute top-0 mt-8 right-7 md:right-[unset] md:left-[50%] md:translate-x-[-50%] h-[41px] w-[162px]"></a>
...
<section class="leo-menu-modal fixed top-0 left-0 h-[100dvh] w-full z-[15] bg-maroon mobile-menu" style="display:none;">
  <img src="@asset('images/charcoal-bg-high-res.png')" loading="eager" alt="" class="leo-menu-modal-bg-img object-cover absolute top-0 left-0 w-full h-full z-[16]">
  ...
  <div class="leo-top-bar relative z-[18] flex justify-between items-center h-[105px] px-7 md:px-10">
    <img src="@asset('images/icons/close-white.svg')" loading="lazy" alt="" class="leo-close-menu-modal md:ml-[40px] md:mt-[40px] hover:cursor-pointer" />
    @if(!empty($header_enquiry_button))
            ...
            @foreach ($header_videos as $item)
              <div id="{!! $item['video_id'] !!}" class="leo-menu-modal-media-wrapper-inner relative h-full w-full @if($iteration > 1) hidden @endif">
                ...
                @if(!empty($item['video_poster_image']))
                  <img src="{!! $item['video_poster_image']['url'] !!}" loading="eager" alt="{!! $item['video_poster_image']['alt'] !!}" class="leo-menu-modal-video__poster absolute z-[11] top-0 left-0 w-full h-full object-cover object-center">
                @endif
              </div>
              @php $iteration += 1 @endphp
            @endforeach
          ...
          @if(!empty($header_videos))
            <div id="{!! $header_videos[0]['video_id'] !!}" class="leo-menu-modal-media-wrapper-inner relative h-full w-full">
              ...
              @if(!empty($header_videos[0]['video_poster_image']))
                <img src="{!! $header_videos[0]['video_poster_image']['url'] !!}" loading="eager" alt="{!! $header_videos[0]['video_poster_image']['alt'] !!}" class="leo-menu-modal-video__poster absolute z-[11] top-0 left-0 w-full h-full object-cover object-center">
              @endif
            </div>
          @endif
        </div>
